...
int main()
	{
		...
		aBag.add("change");
	//change
		//this will make sure that the bag did not add the last item, which would have exceeded the bag's capacity
		if (aBag.getFrequencyOf("change") != 0)
			cout << "Bag added an item that should have exceeded its capacity; not good" << endl;
		else
			cout << "Bag did not add an item that would have exceeded its capacity; this is good" << endl;
	...
	//change
		//check that the isEmpty function works properly
		if (aBag.isEmpty()) 
			cout << "isEmpty function works properly";
		else 
			cout << "is Empty function does not work properly";
		
	//change	
		//create a second bag
		ArrayBag<string> bBag;	
		//add stuff to aBag & bBag & make sure that everything adds correctly
		aBag.add("AAAA");
		bBag.add("BBBB");
		if (bBag.contains("AAAA"))
			cout << "String was added to the wrong bag; not good" << endl;
		if (aBag.contains("BBBB"))
			cout << "String was added to the wrong bag; not good" << endl;
